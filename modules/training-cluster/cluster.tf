...
chmod +x ./kubectl
while true; do
    if [ $(./kubectl --kubeconfig <(echo $KUBECONFIG | base64 --decode) get node -l node-role.kubernetes.io/control-plane=true -o name --no-headers | wc -l) -eq $NUM_CONTROLPLANE ]; then break; fi;
done
EOH
    interpreter = ["/bin/bash", "-c"]
    environment = {
